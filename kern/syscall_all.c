...
int sys_set_env_status(u_int envid, u_int status) {
	...
	if (status == ENV_RUNNABLE) TAILQ_INSERT_TAIL(&env_sched_list, env, env_sched_link);
	else TAILQ_REMOVE(&env_sched_list, env, env_sched_link);
...
int sys_msg_status(u_int msgid) {
	...
	/* Your Code Here (3/3) */
	m = msgs + MSGX(msgid);
	u_int generated = msg2id(m);
	if (msgid == generated) return m->msg_status;
	else if (msgid < generated) {
		m->msg_status = MSG_RECV;
		return MSG_RECV;
	} else return -E_INVAL;
}
